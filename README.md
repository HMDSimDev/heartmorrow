...
# Heartmorrow — Local-First AI-Powered Dating Simulator 💞
...
small local server, and that server talks to the endpoint you point it at: any
OpenAI-API-compatible server (LM Studio, Ollama, llama.cpp, vLLM, etc.), one of the native
LM Studio / Ollama / KoboldCpp connections, or hosted Anthropic with an API key. Your
characters, saves, art, and API keys never leave your computer.
...
- **Energy economy.** Each day gives you a small action pool — **3 actions (4 on
  weekends)**. A date, a hangout, a work shift, time spent with someone (Together), and a
  minigame each spend one — the heaviest shift takes two — while texting is always free.
  Time-of-day phases aren't just decor — they advance as
...
- **Or take the stakes off.** A **hangout** is the same live conversation with the date
  apparatus switched off — no turn-by-turn scoring, no walkouts, no defining the
  relationship. It still ends with an honest read: memories form and the quiet stats
  (comfort, trust) grow, because people fall for each other on dates, not by killing an
  afternoon.
- **Pick your difficulty.** A **Gentle / Normal / Harsh** setting scales how hard
  consequences land — never what characters say or where a date opens, only how far your
  wins and missteps move things.
...
- **Mail** — a read-only inbox of in-world mail from companies and strangers — though
  someone you've dated may send a postcard home while they're away.
...
- **Constellation** — a touch star-map of every relationship in the world: you at the warm
  center, threads out to everyone you've met, colored by the kind of tie.
- **Moments** — a polaroid scrapbook of your relationship highlights.
- **Endings** — your gallery of earned epilogues.
- **Work** — a job board of flat shifts and playable skill jobs; four **career skills**
  (Service, Hustle, Craft, Knowledge) level up with use, raising pay and unlocking
  heavier work.
...
Six **bonding games** grow your bond while earning a little money: **Memory Match**,
...
and playing someone's *favorite* game — picked by their strongest trait — lands warmer.)

Three more are **skill jobs** in the Work app, paid purely in coin: swing on the beat at the
**Woodlot**, transcribe the day's dispatch cleanly at the **Copy Desk**, and haggle scrounged
goods at the **Flip** — where buyers hide a walk-away price, their tells honestly hint at it,
and you pocket only the margin above what a piece is worth.
...
- **Anniversaries are remembered.** A full season after a first date — or the commitment
  that replaced it — the *character* texts you that morning, the Almanac marks the day with
  a heart, and taking them out that evening is worth extra.
...
- **Property** — lease in-world places (rent comes due on a real schedule; come up short and
  ...
  for a relationship buff — a free venue, with owned places giving the full lift and leased
  ones half.
...
![Screenshot of the Heartmorrow People page — the world's cast.](https://github.com/Heartmorrow/heartmorrow-sim/raw/master/gh/5.png)
...
3. Go to **Date**, pick a character and a location, and start talking. (First-use
   walkthroughs introduce the big systems as you meet them.)
4. Stuck on anything? The in-app **Help** tab is a full how-to-play field guide.
...
Heartmorrow works with any endpoint that speaks the OpenAI Chat Completions API, and ships
**native connections** for a few favorites. Common choices and their default URLs:
...
Beyond the OpenAI-compatible mode, Settings offers native connections for **LM Studio**
(richer model details and per-response speed stats), **Ollama** (a Thinking control — turn a
model's reasoning on or off, or set its depth), and **KoboldCpp** (a raw text engine; pick
the instruction template that matches your loaded model — Alpaca, ChatML, Llama 3, and so
on) — plus hosted **Anthropic** with an API key.
